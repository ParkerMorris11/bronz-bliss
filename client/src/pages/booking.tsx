      ...
      <div className="bg-white border-b">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-2.5">
          ...
          <div>
            ...
            {settings?.address && (
              <p className="text-[10px] text-stone-600 leading-tight mt-0.5">{settings.address}</p>
            )}
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2 mb-3">
              {steps.map((s, i) => (
                <div key={s} className="flex items-center gap-2 flex-1 last:flex-none">
                  ...
                  <span className={`text-xs ${i === stepIndex ? "font-medium" : "text-stone-600"}`}>
                    {stepLabels[i]}
                  </span>
        ...
        {step === "service" && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-stone-600">Choose the service you'd like to book</p>
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {svc.description && (
                        <p className="text-xs text-stone-600 mt-0.5">{svc.description}</p>
                      )}
                    </div>
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-stone-600 mt-0.5">{selectedService.name} · {selectedService.duration} min · ${selectedService.price}</p>
              </div>
            </div>
            ...
            <div className="rounded-xl border bg-white p-4">
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {["M","T","W","T","F","S","S"].map((d, i) => (
                  <div key={i} className="text-[10px] font-medium text-stone-600 py-1">{d}</div>
                ))}
                {grid.flat().map((d, i) => {
                  const iso = toIso(d);
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-lg py-1.5 text-xs font-medium transition-colors ${
                        ...
                        isPast || !isThisMonth ? "text-stone-400 cursor-default" :
            ...
            <div>
              ...
              {!availability ? (
                ...
                <div className="rounded-xl border border-dashed p-6 text-center">
                  <CalendarDays className="w-6 h-6 text-stone-400 mx-auto mb-2" />
                  <p className="text-sm text-stone-600">Closed on this day</p>
                </div>
              ) : availability.slots.length === 0 ? (
                <div className="rounded-xl border border-dashed p-6 text-center">
                  <p className="text-sm text-stone-600">No times available</p>
                  <p className="text-xs text-stone-500 mt-1">Try a different date</p>
                </div>
              ) : (
                <div className="grid grid-cols-4 gap-2">
        ...
        {step === "done" && confirmation && (
          <div className="text-center py-8 space-y-4">
            ...
            <div className="rounded-xl bg-white border p-4 text-left space-y-2 max-w-xs mx-auto">
              <p className="text-xs font-semibold uppercase tracking-wide text-stone-600">Your Appointment</p>
              ...
              <p className="text-sm text-stone-600">{formatTime(confirmation.time)}</p>
              {settings?.depositRequired && settings.depositAmount && (
